...
void voltage_init_adc(void) {
	...
	XMC_VADC_CHANNEL_CONFIG_t  channel_fb_config = {
		...
		.result_reg_number          =  VOLTAGE_FB_RESULT_REG,                  // GxRES[10] selected
		...
		.alias_channel              =  VOLTAGE_FB_ALIAS                        // Channel is Aliased
	};

	XMC_VADC_CHANNEL_CONFIG_t  channel_vext_config = {
		.input_class                =  XMC_VADC_CHANNEL_CONV_GLOBAL_CLASS0,    // Global ICLASS 0 selected
		.lower_boundary_select 	    =  XMC_VADC_CHANNEL_BOUNDARY_GROUP_BOUND0,
		.upper_boundary_select 	    =  XMC_VADC_CHANNEL_BOUNDARY_GROUP_BOUND0,
		.event_gen_criteria         =  XMC_VADC_CHANNEL_EVGEN_NEVER,           // Channel Event disabled
		.sync_conversion  		    =  0,                                      // Sync feature disabled
		.alternate_reference        =  XMC_VADC_CHANNEL_REF_INTREF,            // Internal reference selected
		.result_reg_number          =  VOLTAGE_VEXT_RESULT_REG,                // GxRES[10] selected
		.use_global_result          =  0, 				                       // Use Group result register
		.result_alignment           =  XMC_VADC_RESULT_ALIGN_RIGHT,            // Result alignment - Right Aligned
		.broken_wire_detect_channel =  XMC_VADC_CHANNEL_BWDCH_VAGND,           // No Broken wire mode select
		.broken_wire_detect		    =  0,    		                           // No Broken wire detection
		.bfl                        =  0,                                      // No Boundary flag
		.channel_priority           =  0,                   		           // Lowest Priority 0 selected
		.alias_channel              =  VOLTAGE_VEXT_ALIAS                      // Channel is Aliased
	};
	...
	XMC_VADC_GROUP_ChannelInit(VADC_G0, VOLTAGE_FB_CHANNEL_NUM,   &channel_fb_config);
	XMC_VADC_GROUP_ChannelInit(VADC_G0, VOLTAGE_VEXT_CHANNEL_NUM,  &channel_vext_config);

	// Initialize for configured result registers
	XMC_VADC_GROUP_ResultInit(VADC_G0, channel_fb_config.result_reg_number,   &channel_result_config);
	XMC_VADC_GROUP_ResultInit(VADC_G0, channel_vext_config.result_reg_number,  &channel_result_config);

	XMC_VADC_GLOBAL_BackgroundAddChannelToSequence(VADC, 0, VOLTAGE_FB_CHANNEL_NUM);
	XMC_VADC_GLOBAL_BackgroundAddChannelToSequence(VADC, 0, VOLTAGE_VEXT_CHANNEL_NUM);
...
void voltage_init(void) {
	...
	XMC_GPIO_Init(VOLTAGE_VEXT_PIN,  &input_default_config);
	XMC_GPIO_Init(VOLTAGE_FB_PIN,   &input_default_config);

	voltage_init_adc();
}

void voltage_tick(void) {
	const uint32_t result_vext = XMC_VADC_GROUP_GetDetailedResult(VADC_G0, VOLTAGE_VEXT_RESULT_REG);
	const uint32_t result_fb   = XMC_VADC_GROUP_GetDetailedResult(VADC_G0, VOLTAGE_FB_RESULT_REG);

	if((result_vext & (1 << 31))) {
		voltage.adc_sum_vext += (result_vext & 0xFFFF);
		voltage.adc_count_vext++;
	}

	if((result_fb & (1 << 31))) {
		voltage.adc_sum_fb += (result_fb & 0xFFFF);
		voltage.adc_count_fb++;
	}

	if(system_timer_is_time_elapsed_ms(voltage.adc_last_time, 100)) {
		voltage.adc_last_time = system_timer_get_ms();

		// VEXT voltage: 100k/10k => 3300*11/(4095*4) => 605/273
		if(voltage.adc_count_vext != 0) {
			voltage.voltage = (voltage.adc_sum_vext*605)/(voltage.adc_count_vext*273);
		}

		// FB current:  6250/(4095*4) => 625/1638
		if(voltage.adc_count_fb != 0) {
			voltage.current = (voltage.adc_sum_fb*625)/(voltage.adc_count_fb*1638);
		}

		voltage.adc_count_vext = 0;
		voltage.adc_count_fb   = 0;
		voltage.adc_sum_vext   = 0;
		voltage.adc_sum_fb     = 0;
